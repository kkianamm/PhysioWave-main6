...
def sliding_window_emg(emg_array, wsize=512, step=256):
    """
    Segment emg_array (N, channels) using sliding windows.
    
    Args:
        emg_array: Input EMG data of shape (N, channels)
        wsize: Window size (default 512 samples)
        step: Step size between windows (default 256 samples)
...
if __name__ == "__main__":
    ...
    with h5py.File(temp_h5, "w") as f:
        f.create_dataset("data", shape=(0, 8, 512),
                         maxshape=(None, 8, 512),
                         compression="gzip",
                         dtype=np.float32)
    ...
    for subj_folder in sorted(os.listdir(data_root)):
        ...
        for mat_file in sorted(os.listdir(subj_path)):
            ...
            emg_segmented = sliding_window_emg(emg_normalized, wsize=512, step=256)
            ...
            with h5py.File(temp_h5, "a") as f:
                ...
                new_shape = (old_shape[0] + emg_segmented.shape[0], 8, 512)
    ...
    if total_samples > 0:
        ...
        print(f"\nData format:")
        print(f"  - 8 channels")
        print(f"  - 512 samples per window at 2000 Hz")
